...
import {
  ...
  AddProblemsToBookmarkRequest
} from "../types/index.js";

export const createBookmark = async (
  req: Request<{}, any, CreateBookmarkRequest>,
  res: Response
): Promise<void> => {
...
export const addProblemsToBookmark = async (
  req: Request<{ bookmarkId: string }, any, AddProblemsToBookmarkRequest>,
  res: Response
): Promise<void> => {
  const { bookmarkId } = req.params;
  ...
  const connectData = problemIds.map((problemId: string) => ({ id: problemId }));
  try {
...
export const deleteProblemsFromBookmark = async (
  req: Request<{ bookmarkId: string }, any, AddProblemsToBookmarkRequest>,
  res: Response
): Promise<void> => {
  const { bookmarkId } = req.params;
  ...
  const disconnectData = problemIds.map((problemId: string) => ({ id: problemId }));
  try {
    const bookmark = await prisma.bookmark.update({
      where: { id: bookmarkId, userId: req.user!.id },
